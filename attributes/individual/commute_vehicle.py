...
def read_vehicle_activity() -> pd.DataFrame:
    data_path = os.path.join(
            ...
            '../../datasources/individual/commute/dopravni_prostredek.csv'
    )
...
def fit_vehicle_activity(df_synth_pop) -> pd.DataFrame:
    df = read_vehicle_activity().groupby(['age_group', 'gender', 'ea_school_work_2', 'vehicle_activity'])['count'].sum().reset_index()
    ...
    margins_gender_age_activity_type = synthetic_population_to_contingency(df_synth_pop, ["gender", "age_group", "ea_school_work_2"])["count"]
    ...
    frequencies = df['vehicle_activity'].unique()
    ...
    kompletni_index_marginal = pd.MultiIndex.from_product(
        ...
        names=["gender", "age_group", "ea_school_work_2"]
    )
    ...
    df_vsechny = pd.DataFrame(vsechny_kombinace, columns=['age_group', 'gender', 'ea_school_work_2', 'vehicle_activity'])

    df = pd.merge(df_vsechny, df, on=['age_group', 'gender', 'ea_school_work_2', 'vehicle_activity'], how='left')
    df['count'] = df['count'].fillna(0.000000000001)
    df["count"] = df["count"].astype(float)
    df = df.groupby(['age_group', 'gender', 'ea_school_work_2', 'vehicle_activity'])['count'].sum().reset_index()
    print(df.to_string())
    # =======================================================
    df_fitted = ipfn.ipfn(
            df.copy().astype({'count': 'float'}),
            aggregates=[margins_age, margins_gender, margins_ea_school_work_2, margins_activity, margins_gender_age_activity_type],
            dimensions=[['age_group'], ['gender'], ['ea_school_work_2'], ['vehicle_activity'], ['gender','age_group', 'ea_school_work_2']],
            weight_col='count'
    ).iteration()
    ...
    validate_fitted_distribution(df_fitted, margins_gender_age_activity_type, ['gender', 'age_group', 'ea_school_work_2'], name)
