...
def read_vehicle_activity() -> pd.DataFrame:
    data_path = os.path.join(
            ...
            '../../datasources/individual/commute/dopravni_prostredek_1.csv'
    )
...
def fit_vehicle_activity(df_synth_pop) -> pd.DataFrame:
    df = read_vehicle_activity().groupby(['age_group', 'gender', 'ea_school_work_2', 'category_place_activity', 'vehicle_activity'])['count'].sum().reset_index()
    ...
    margins_gender_age_activity_category_type = synthetic_population_to_contingency(df_synth_pop, ["gender", "age_group", "ea_school_work_2", 'category_place_activity'])["count"]
    ...
    kompletni_index_marginal = pd.MultiIndex.from_product(
        ...
        names=["gender", "age_group", "ea_school_work_2", "category_place_activity"]
    )
    ...
    vsechny_kombinace = list(itertools.product(ages, genders, activities, categories_place_activity, vehicles))
    df_vsechny = pd.DataFrame(vsechny_kombinace, columns=['age_group', 'gender', 'ea_school_work_2', 'category_place_activity', 'vehicle_activity'])

    df = pd.merge(df_vsechny, df, on=['age_group', 'gender', 'ea_school_work_2', 'category_place_activity', 'vehicle_activity'], how='left')
    df['count'] = df['count'].fillna(0.000000000001)
    maska_nesmysl = (df['category_place_activity'] == 'not_moving') & (df['vehicle_activity'] != 'not_moving')
    df.loc[maska_nesmysl, 'count'] = 0
    df["count"] = df["count"].astype(float)
    df = df.groupby(['age_group', 'gender', 'ea_school_work_2', 'category_place_activity', 'vehicle_activity'])['count'].sum().reset_index()
    print(df.to_string())
    # =======================================================
    df_fitted = ipfn.ipfn(
            df.copy().astype({'count': 'float'}),
            aggregates=[margins_age, margins_gender, margins_ea_school_work_2, margins_category_place_activity, margins_activity, margins_gender_age_activity_category_type],
            dimensions=[['age_group'], ['gender'], ['ea_school_work_2'], ['category_place_activity'], ['vehicle_activity'], ['gender','age_group', 'ea_school_work_2', 'category_place_activity']],
            weight_col='count'
    ).iteration()
    ...
    validate_fitted_distribution(df_fitted, margins_gender_age_activity_category_type, ['gender', 'age_group', 'ea_school_work_2'], name)
