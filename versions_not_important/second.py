...
def calculate_age_range_from_gap(age: Union[int, float], gap_start: Union[int, float], gap_end: Union[int, float]) -> tuple[Union[int, float], Union[int, float]]:
...
class HouseholdType:
    ...
    def match_adults_with_children(self, parents: list[list[tuple[str, int, str]]], children: list[list[str]],
        ...
        def get_c_ages(c_cluster):
            ages = []
            for c in c_cluster:
                val = self.df_synth_pop.loc[c, 'age']
                ages.append(int(val.max()) if hasattr(val, 'max') else int(val))
            return ages
        ...
        def score_household(p_ages, c_ages):
            ...
            for p_age in p_ages:
                gap_oldest, gap_youngest = p_age - max_c, p_age - min_c
                if 12 <= gap_oldest and gap_youngest <= 60: return 0
                penalty = 0
                if gap_oldest < 15: penalty += (15 - gap_oldest) * 50
                if gap_youngest > 50: penalty += (gap_youngest - 50) * 50
                if penalty < best_penalty: best_penalty = penalty
            return best_penalty

        def get_current_stats(households_list):
            p_count, total_s = 0, 0
            for hh in households_list:
                s = score_household(hh['parent']['ages'], get_c_ages(hh['child']))
                if s > 0:
                    p_count += 1
                    total_s += s
            return p_count, total_s

        # --- PRVOTNÍ PŘIŘAZENÍ (Zůstává stejné) ---
        ...
        for c_cluster in unmatched_children:
            ...
            for i, p_info in enumerate(unmatched_parents):
                if any(18 <= (p_age - max_c) and (p_age - min_c) <= 45 for p_age in p_info['ages']):
                    matched_households.append({'parent': unmatched_parents.pop(i), 'child': c_cluster})
                    found = True; break
            if not found: remaining.append(c_cluster)
        
        for c_cluster in remaining:
            if not unmatched_parents: break
            matched_households.append({'parent': unmatched_parents.pop(0), 'child': c_cluster})

        # =========================================================
        # FÁZE 2: AGRESIVNÍ OPTIMALIZACE S TREZOREM NA MINIMUM
        # =========================================================
        # Inicializace trezoru
        best_overall_households = copy.deepcopy(matched_households)
        min_problems_ever, min_score_ever = get_current_stats(matched_households)
        
        improved = True
        cycles = 0
        
        # HLAVNÍ CYKLUS
        while (improved or cycles < 10) and cycles < 100:
            if min_problems_ever == 0:
                print("Dosáhnuto dokonalého stavu (0 problémů). Končím předčasně.")
                break

            improved = False
            cycles += 1
            
            # 1. SHAKE (Třesení - rozbití současného stavu)
            for _ in range(10):
                if len(matched_households) < 2: break
                idx1, idx2 = random.sample(range(len(matched_households)), 2)
                matched_households[idx1]['child'], matched_households[idx2]['child'] = \
                    matched_households[idx2]['child'], matched_households[idx1]['child']
            
            # B) CHIRURGICKÁ OPTIMALIZACE (Cílené prohazování) uvnitř cyklu
            sub_improved = True
            while sub_improved:
                sub_improved = False
                problematic = [i for i, hh in enumerate(matched_households) 
                              if score_household(hh['parent']['ages'], get_c_ages(hh['child'])) > 0]
                
                if not problematic: break

                for i in problematic:
                    score_i_before = score_household(matched_households[i]['parent']['ages'], 
                                                     get_c_ages(matched_households[i]['child']))
                    
                    for j in range(len(matched_households)):
                        if i == j: continue
                        child_i = matched_households[i]['child']
                        child_j = matched_households[j]['child']
                        
                        score_j_before = score_household(matched_households[j]['parent']['ages'], get_c_ages(child_j))
                        total_before = score_i_before + score_j_before

                        score_i_after = score_household(matched_households[i]['parent']['ages'], get_c_ages(child_j))
                        score_j_after = score_household(matched_households[j]['parent']['ages'], get_c_ages(child_i))
                        total_after = score_i_after + score_j_after

                        # AGRESIVNÍ LOGIKA
                        if total_after < total_before and score_j_after < 5000:
                            matched_households[i]['child'] = child_j
                            matched_households[j]['child'] = child_i
                            sub_improved = True
                            improved = True # Něco se v tomto velkém cyklu zlepšilo
                            break 
                    if sub_improved: break

            # --- KONTROLA REKORDU (TREZOR) ---
            cur_problems, cur_score = get_current_stats(matched_households)
            if cur_problems < min_problems_ever or (cur_problems == min_problems_ever and cur_score < min_score_ever):
                min_problems_ever = cur_problems
                min_score_ever = cur_score
                best_overall_households = copy.deepcopy(matched_households)
                print(f"!!! NOVÝ REKORD V CYKLU {cycles}: {min_problems_ever} problémů (Score: {min_score_ever})")

        # OBNOVENÍ TOHO NEJLEPŠÍHO CO JSME KDY NAŠLI
        matched_households = best_overall_households
        print(f"Končím. Nejlepší nalezený stav: {min_problems_ever} problémů.")

        # =========================================================
        # FÁZE 3: ZÁPIS
        # =========================================================
        for hh in matched_households:
            p_ages, c_ages = hh['parent']['ages'], get_c_ages(hh['child'])
            score = score_household(p_ages, c_ages)

            p_ids = [p[0] for p in hh['parent']['p_cluster']]
            c_ids = hh['child']
            hh_id = f"SSH{id_offset:06d}"
            gn = getattr(self, 'current_group_name', 'Neznámý okrsek')
            ht = getattr(self, 'hh_type', 'Neznámý typ')

            if score > 0:
                warnings.warn(f"Datový extrém: Biologický fit nelze opravit. Rodiče: {p_ages}, Děti: {c_ages}.")
                msg = f"NEOPRAVENO (Rodič-Dítě): Rodiče {p_ages}, Děti {c_ages}, Skóre {score}"
                print(f"!!! {msg}") # Pořád uvidíš v terminálu
                msg = (f"{gn} | Domácnost: {hh_id} | HH: {hh_id} | Skóre: {score} | "
                       f"Rodiče ID: {p_ids} (Věk: {p_ages}) | "
                       f"Děti ID: {c_ids} (Věk: {c_ages})")
                logging.warning(msg) # Zapíše se do souboru
            
            household_members = [p[0] for p in hh['parent']['p_cluster']] + hh['child']
            self.create_household_with_id(self.get_position_for_name('child'), id_offset, household_members)
            
            bio_gap = min(p_ages) - max(c_ages)
            print(f"Matched parent_ages={p_ages} to child_ages={c_ages} (BioGap: {bio_gap}) ID: SSH{id_offset:06d}")
            id_offset += 1
    ...
    def find_opposite_gender_replacement_for_candidate(self, wrong_candidate, msk, position: str) -> Optional[str]:
        """
    ...
    def group_children(self, msk, child_position: dict) -> List[List[str]]:
        ...
        print(f"Grouping children into sets of {n_children} (Chirurgická optimalizace)")

        # 1. Příprava poolu dětí
        pool_df = self.df_synth_pop[(self.df_synth_pop[self.household_position_column].isin(position)) & msk]
        
        if n_children <= 1:
            ids = pool_df.index.tolist()
            self.sampled_agents.extend(ids)
            return [[child_id] for child_id in ids]

        # 2. Úvodní náhodné rozdělení
        pool_ids = pool_df.sample(frac=1).index.tolist()
        clusters = []
        for i in range(0, len(pool_ids), n_children):
            cluster = pool_ids[i:i + n_children]
            clusters.append(cluster)

        # pomocné funkce
        def get_age(child_id):
            val = pool_df.loc[child_id, 'age']
            return int(val.max()) if hasattr(val, 'max') else int(val)

        def score_cluster(cluster):
            if len(cluster) < 2: return 0
            ages = [get_age(c) for c in cluster]
            gap = max(ages) - min(ages)
            
            if gap == 0: return 20     # Trest pro dvojčata
            if 1 <= gap <= 4: return 0 # Zlatá zóna
            if 5 <= gap <= 10: return gap * 2
            if gap <= 30: return gap * 10
            return 5000                # Biologický nesmysl

        # =========================================================
        # CHIRURGICKÁ OPTIMALIZACE SOUROZENCŮ
        # =========================================================
        improved = True
        cycles = 0
        
        while improved and cycles < 50: # U sourozenců stačí méně cyklů
            improved = False
            cycles += 1
            
            # Najdeme indexy problematických balíčků
            problematic_indices = [idx for idx, c in enumerate(clusters) if score_cluster(c) > 0]
            
            if not problematic_indices:
                break

            random.shuffle(problematic_indices) # Abychom nezačínali vždy stejným

            for i in problematic_indices:
                score_i_before = score_cluster(clusters[i])
                
                # Zkusíme prohodit každé dítě z balíčku 'i'
                for i_child_idx in range(len(clusters[i])):
                    
                    # Proti každému balíčku 'j' v okrsku
                    for j in range(len(clusters)):
                        if i == j: continue
                        if len(clusters[i]) != len(clusters[j]): continue
                        
                        # Zkusíme prohodit s každým dítětem v balíčku 'j'
                        for j_child_idx in range(len(clusters[j])):
                            
                            # Simulace prohozu
                            child_i = clusters[i][i_child_idx]
                            child_j = clusters[j][j_child_idx]
                            
                            # Vytvoříme testovací verze
                            test_cluster_i = list(clusters[i])
                            test_cluster_j = list(clusters[j])
                            test_cluster_i[i_child_idx] = child_j
                            test_cluster_j[j_child_idx] = child_i
                            
                            score_i_after = score_cluster(test_cluster_i)
                            score_j_after = score_cluster(test_cluster_j)
                            
                            # ROZHODUJÍCÍ LOGIKA:
                            # 1. Balíček 'j' nesmí být po prohozu horší (pokud byl na 0, musí zůstat na 0)
                            # 2. Balíček 'i' si musí polepšit
                            if score_j_after <= score_cluster(clusters[j]) and score_i_after < score_i_before:
                                clusters[i] = test_cluster_i
                                clusters[j] = test_cluster_j
                                score_i_before = score_i_after
                                improved = True
                                # print(f"   [Sourozenci] Prohoz i:{i} <-> j:{j} | Nové skóre: {score_i_after}")
                            
                            if score_i_before == 0: break
                        if score_i_before == 0: break
                    if score_i_before == 0: break

        # 4. ZÁPIS A KONTROLA
        for cluster in clusters:
            self.sampled_agents.extend(cluster)
            score = score_cluster(cluster)
            if score >= 5000:
                ages = [get_age(c) for c in cluster]
                gn = getattr(self, 'current_group_name', 'Neznámý okrsek')
                msg = f"{gn} | Sourozenci ID: {cluster} | Věky: {ages} | Skóre: {score}"
                # msg = f"NEOPRAVENO (Sourozenci): Věky v balíčku {ages}, Skóre {score}"
                logging.warning(msg)
                warnings.warn(f"Nepodařilo se biologicky opravit sourozence: {ages}")
...
class HouseholdGrouper:
    ...
    def run(self):
        ...
        logging.basicConfig(
            filename='biogap_report.log', 
            level=logging.WARNING,
            format='%(asctime)s - %(levelname)s - %(message)s',
            encoding='utf-8'
        )

        # # Pokud chceš soubor při každém startu vyčistit:
        # with open('biogap_report.log', 'a') as f:
        #     f.write("--- BIOGAP OPTIMALIZACE REPORT ---\n")
        # print("Starting household grouping process...")
        ...
        for household_type in self.household_types:
            ...
            for group_name, _ in self.df_synth_pop.groupby(self.group_by):
                if isinstance(group_name, tuple):
                    clean_name = " | ".join(map(str, group_name))
                else:
                    clean_name = str(group_name)

                # Uložíme vyčištěný název
                household_type.current_group_name = clean_name
