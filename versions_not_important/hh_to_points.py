...
def score_v3(hh_size, hh_full_floor, b_target_pop, b_res_floors, has_free_unit):
    penalty = abs(b_target_pop - hh_size) * 1000
    if not has_free_unit: penalty += 500
    if not check_floor_match(hh_full_floor, b_res_floors): penalty += 50
    return penalty
...
def extract_building_table(gpkg_path: str) -> pd.DataFrame:
    ...
    # Načtení základních sloupců včetně npodl
    ...
    def get_res_floors_smart(row):
        # 1. Zkusíme najít patra, kde je explicitně "bydlení"
        res_floors = [int(pattern.match(c).group(1)) for c in np_cols if str(row[c]).lower() == 'bydlení']
        
        # 2. POJISTKA: Pokud jsme nic nenašli, ale budova má mít lidi nebo byty
        if not res_floors and (row['bobyosl21'] > 0 or row['sum_byt'] > 0):
            # Pokud víme aspoň počet pater z npodl, uděláme z nich všechna obytná
            if row['npodl'] > 0:
                res_floors = list(range(1, row['npodl'] + 1))
            else:
                # Pokud i npodl je NULL/0, uděláme z toho jednopatrový dům (tvůj požadavek)
                res_floors = [1]
    ...
    # Odfiltrujeme budovy, které po všech pojistkách stále nemají kde bydlet (skutečné garáže/trafostanice)
    df = df[df['res_floors'].map(len) > 0].copy()
    
    return df

def assign_households_locally(df_hh, df_buildings):
    print("Phase 1: Hlavní ubytování...")
    ...
    for mc in all_mc:
        if mc not in hh_dict: continue
        mc_b = df_buildings[df_buildings['kod'] == mc]
        
        for h_type, b_code in [('rd','07'), ('ostatni_budovy','ostatni'), ('bd','06')]:
            b_sub = mc_b[mc_b['zpvybu']==b_code] if b_code!='ostatni' else mc_b[~mc_b['zpvybu'].isin(['06','07'])]
            
            for _, b in b_sub.iterrows():
                fid, slots, target = b['TARGET_FID'], b['sum_byt'], b['bobyosl21']
                res_floors = b['res_floors']
                
                for _ in range(slots):
                    candidates = hh_dict.get(mc, {}).get(h_type, [])
                    if not candidates: 
                        candidates = [i for t in hh_dict.get(mc, {}).values() for i in t]
                    if not candidates: break

                    sample = random.sample(candidates, min(len(candidates), 500))
                    best_idx, min_p = None, float('inf')
                    for idx in sample:
                        p = score_v3(hh_info[idx]['hh_size'], hh_info[idx]['building_floor'], 
                                     target - current_pop[fid], res_floors, current_units[fid] < slots)
                    ...
                    for t in hh_dict[mc]:
                        if best_idx in hh_dict[mc][t]:
                            hh_dict[mc][t].remove(best_idx); break
    return df_hh

def handle_leftovers_v3(df_hh, df_buildings):
    unassigned = df_hh[df_hh['assigned_TARGET_FID'].isna()].index
    if len(unassigned) == 0: return df_hh
    print(f"Phase 2: Řeším {len(unassigned)} přebytků...")
    ...
    # Tady byla ta chyba - set_index musí být na unikátním sloupci (TARGET_FID)
    b_info = df_buildings.set_index('TARGET_FID').to_dict('index')
    mc_map = {}
    for fid, d in b_info.items(): mc_map.setdefault(d['kod'], []).append(fid)

    for idx in unassigned:
        row = df_hh.loc[idx]
        mc, h_size, h_floor_str = row['neighb_code'], row['hh_size'], row['building_floor']
        fids = mc_map.get(mc, [])
        if not fids: continue

        underfilled = [f for f in fids if current_pop.get(f, 0) < b_info[f]['bobyosl21']]
        if underfilled:
            fid = random.choice(underfilled)
...
def optimize_by_swapping_v2(df_hh, df_buildings, iterations=50000):
    print(f"Phase 3: Optimalizace ({iterations} iterací)...")
    ...
    for i in range(iterations):
        ...
        err_after = (new_p1 - target_pop[fid1])**2 + (new_p2 - target_pop[fid2])**2
        ...
        if i % 10000 == 0: print(f"   ... iterace {i}")
    return df_hh
...
# FINÁLNÍ PROPOJENÍ: Aby v CSV bylo vidět i ruianso_id
