...
class HouseholdType:
    ...
    def match_adults_with_children(self, parents, children, id_offset):
        import random
        import warnings
        import logging
        ...
        def get_c_ages(c_cluster):
            return [int(self.df_synth_pop.loc[c, 'age']) for c in c_cluster]

        def is_valid(p_ages, c_ages):
            return any(
                15 <= (p - max(c_ages)) <= 60
                for p in p_ages
            )
        ...
        empirical_gaps = []
        for gap, weight in self.parent_child_age_distribution.items():
            start, end = map(int, str(gap).split("-"))
            empirical_gaps.append((start, end, weight))

        def empirical_penalty(p_ages, c_ages):
            if not c_ages:
                return 0
            ...
            for p in p_ages:
                ...
                dist = min(
                    0 if start <= gap <= end else min(abs(gap - start), abs(gap - end))
                    for start, end, _ in empirical_gaps
                )
        ...
        def biological_penalty(p_ages, c_ages):
            if not c_ages:
                return 0
            ...
            for p_age in p_ages:
                ...
                if 15 <= gap_oldest and gap_youngest <= 50:
                    return 0

                penalty = 0
                if gap_oldest < 15:
                    penalty += (15 - gap_oldest) * 50
                if gap_youngest > 50:
                    penalty += (gap_youngest - 50) * 50

                best_penalty = min(best_penalty, penalty)

            return best_penalty

        def total_score(p, c_ages):
            bio = biological_penalty(p["ages"], c_ages)
            emp = empirical_penalty(p["ages"], c_ages)
            return bio + emp + p["usage_penalty"] * 20, bio, emp

        # ---------------------------
        # 1️⃣ Seřazení
        # ---------------------------
        parents = sorted(parents, key=lambda p: np.mean([a[1] for a in p]))
        children = sorted(children, key=lambda c: max(get_c_ages(c)))
        ...
        # 2️⃣ Matching
        # ---------------------------
        for c_cluster in children:
            c_ages = get_c_ages(c_cluster)

            # 🔍 LOG sourozenců (nové)
            if len(c_ages) > 1:
                gap = max(c_ages) - min(c_ages)
                if gap > 15:
                    msg = f"{getattr(self,'current_group_name','?')} | Sourozenci podezřelí | věky {c_ages} | gap={gap}"
                    logging.warning(msg)
                    warnings.warn(msg)

            valid = [
                p for p in parent_pool
                if not p["used"] and is_valid(p["ages"], c_ages)
            ]

            candidates = valid if valid else [p for p in parent_pool if not p["used"]]

            candidates = sorted(candidates, key=lambda p: total_score(p, c_ages)[0])

            WINDOW = min(5, len(candidates))
            chosen = random.choice(candidates[:WINDOW])

            chosen["used"] = True
            chosen["usage_penalty"] += 1
        ...
        # ---------------------------
        # 4️⃣ Lokální optimalizace
        # ---------------------------
        for _ in range(3):
            for i in range(len(households) - 1):
                h1, h2 = households[i], households[i + 1]

                if not h1["child"] or not h2["child"]:
                    continue

                before = (
                    total_score(h1["parent"], get_c_ages(h1["child"]))[0] +
                    total_score(h2["parent"], get_c_ages(h2["child"]))[0]
                )

                after = (
                    total_score(h1["parent"], get_c_ages(h2["child"]))[0] +
                    total_score(h2["parent"], get_c_ages(h1["child"]))[0]
                )

                if after < before:
                    h1["child"], h2["child"] = h2["child"], h1["child"]
        ...
        for hh in households:
            ...
            if c_ids:
                score, bio, emp = total_score(hh["parent"], get_c_ages(c_ids))

                if score > 0:
                    msg = (
                        f"{getattr(self,'current_group_name','?')} | "
                        f"HH SSH{id_offset:06d} | "
                        f"Parents {hh['parent']['ages']} | "
                        f"Children {get_c_ages(c_ids)} | "
                        f"Score={score:.1f} (bio={bio:.1f}, emp={emp:.1f})"
                    )
            ...
            self.create_household_with_id(
                self.get_position_for_name('child'),
                id_offset,
                p_ids + c_ids
            )
    ...
    def group_children(self, msk, child_position: dict) -> List[List[str]]:
        ...
        def score_cluster(cluster):
            ...
            if gap == 0:
                base = 20   # podezřelé (dvojčata ok, ale nechceš jich moc)
            elif gap <= 4:
                base = 0
            elif gap <= 10:
                base = (gap - 4)**2
            ...
            # 👉 NOVÉ: penalizace variability
            std_penalty = np.std(ages) * 2   # váha = 2 (můžeš ladit)

            return base + std_penalty                # Biologický nesmysl
                ...
                for i_child_idx in range(len(clusters[i])):
                    ...
                    for j in range(len(clusters)):
                        ...
                        for j_child_idx in range(len(clusters[j])):
                            ...
                            threshold = np.percentile(scores, 50) + np.std(scores) * 0.5
                            threshold = max(threshold, 5)
                            if ((score_j_after <= score_cluster(clusters[j]) and score_i_after < score_i_before + threshold)
